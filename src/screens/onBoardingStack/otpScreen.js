...
export default function OTPScreen({route,navigation}){
    ...
    const resendOtp =async ()=>{
        ...
        setLoading(true)
        await axios.post(ApiCollection.authController.sendOTp,{"contact":route.params.contact})
        .then(response=>{
    ...
    const verifyOtp =async ()=>{
        setLoading(true)
        await axios.put(ApiCollection.authController.verifyContact,{"otp":`${otpCode0}${otpCode1}${otpCode2}${otpCode3}`, "contact":route.params.contact},{headers:{'Authorization':`Bearer ${route.params.token}`}})
        .then(response=>{
        ...
        .catch(err=>{
            console.log(ApiCollection.authController.verifyContact)
    ...
    return(
        <ScrollView contentContainerStyle={{ flex: 1,justifyContent:'center',alignItems:'center',backgroundColor:'white' }} keyboardShouldPersistTaps='handled'>
            ...
            <Text style={{fontWeight:'700',fontSize:22,marginBottom:15}}>Phone verification</Text>
